'use strict';
  ...
  function updateTimer() {
    ...
    const previous = Number(timer.textContent);
    timer.textContent = String(seconds);
    timer.classList.toggle('danger', seconds <= 10);
    if (seconds > 0 && seconds <= 3 && previous !== seconds) soundWarning();
  ...
  function renderRoundEnd() {
    ...
    if (state.roundScores[0] === state.roundScores[1]) {
      el('roundLeader').textContent = 'Diese Runde endet unentschieden.';
    } else {
      const winner = state.roundScores[0] > state.roundScores[1] ? 0 : 1;
      el('roundLeader').textContent = `${state.teams[winner]} gewinnt diese Runde.`;
    }
