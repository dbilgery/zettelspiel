'use strict';
  ...
  function addPlayer() {
    ...
    if (!name) {
      setError(el('playerError'), 'Bitte gib einen Namen ein.');
      input.focus();
      return;
    }
    if (state.players.some((p) => p.name.localeCompare(name, 'de', { sensitivity: 'accent' }) === 0)) {
      setError(el('playerError'), 'Diesen Namen gibt es bereits.');
      input.select();
      return;
    }
        ...
        <div class="teamMembers">
          ${members.length ? members.map((id) => {
            ...
            return `<div class="member" data-player="${id}" data-from="${teamIndex}">
              ...
              ${state.teamMode === 'manual' ? `<button class="moveMember" type="button" data-move="${id}" data-from="${teamIndex}" aria-label="${escapeHtml(player.name)} ins andere Team verschieben">↔</button>` : ''}
            </div>`;
  ...
  function validateTeams(showMessage = true) {
    ...
    const valid = state.players.length >= 2 &&
    ...
    if (showMessage) setError(el('teamError'), valid ? '' : 'Beide Teams brauchen mindestens einen Spieler.');
    return valid;
  }

  function renderTermSettings() {
    const counts = state.teamMembers.map((team) => team.length);
    el('team1Terms').value = String(state.termsPerPlayer[0]);
    el('team2Terms').value = String(state.termsPerPlayer[1]);
    el('team1TermsValue').textContent = String(state.termsPerPlayer[0]);
    el('team2TermsValue').textContent = String(state.termsPerPlayer[1]);
    el('team1PlayerCount').textContent = `${counts[0]} ${counts[0] === 1 ? 'Spieler' : 'Spieler'}`;
    el('team2PlayerCount').textContent = `${counts[1]} ${counts[1] === 1 ? 'Spieler' : 'Spieler'}`;
    const first = counts[0] * state.termsPerPlayer[0];
    const second = counts[1] * state.termsPerPlayer[1];
    state.termTarget = Math.min(WORD_POOL.length, first + second);
    el('termCalculation').textContent = `${counts[0]}×${state.termsPerPlayer[0]} + ${counts[1]}×${state.termsPerPlayer[1]}`;
    ...
    $$('.member[data-player]').forEach((member) => {
      member.addEventListener('pointerdown', (event) => {
        ...
        drag = {
          ...
          playerId,
          from: Number(member.dataset.from),
          member,
          ghost,
          target: null
        };
  ...
  function renderCollect() {
    ...
    el('poolRemaining').textContent = done ? 'Fertig' : `Noch ${remaining} ${remaining === 1 ? 'Begriff' : 'Begriffe'}`;
    ...
    el('passPhone').hidden = done;
    el('startCollectedGame').hidden = !done;
    if (done) {
      el('termInput').value = '';
      setError(el('termError'));
      setSuccess('Wortpool vollständig.');
    }
  }
  ...
  function addCurrentTerm() {
    ...
    setSuccess('');
    if (!term) {
      setError(el('termError'), 'Bitte gib zuerst einen Begriff ein.');
      input.focus();
      return;
    }
    if (isDuplicateTerm(term)) {
      setError(el('termError'), 'Diesen Begriff gibt es schon.');
      input.select();
      return;
    }
